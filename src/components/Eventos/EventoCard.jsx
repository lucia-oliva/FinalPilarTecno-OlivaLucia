    ...
    <Card variant="outlined" className="eventocard-root">
      ...
      <CardContent className="eventocard-content">
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start" className="eventocard-header">
          ...
          {typeof e.entradasDisponibles !== "undefined" && <CupoBadge disponibles={e.entradasDisponibles} />}
        </Stack>
        <Typography variant="body2" color="text.secondary" className="eventocard-meta">{e.lugar}</Typography>
        {e.descripcion && <Typography variant="body2" className="eventocard-desc">{e.descripcion}</Typography>}
      </CardContent>
      <CardActions className="eventocard-actions">
        <Button component={RouterLink} to={`/eventos/${eid}`} size="small" variant="contained" className="eventocard-button">
          Ver detalle
        </Button>
      </CardActions>
    </Card>
